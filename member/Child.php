...
<body>
  ...
  <!-- adventure-books start -->
	        <div class="adventure-books mt-2 font-tit">
				   	<?php 
		          $sql = "SELECT book_category from books where book_category ='child' limit 1";
		    ...
		    <?php
		           ...
		           //while($row  = mysqli_fetch_assoc($result))
		            {     ?> 
					<h2 class="font-22 border-bottom font-lemon text-uppercase py-3 font-weight-bold" style="color:#F1C40F;">
			         <?= $row['book_category'] ?> 			
					</h2>
				 <?php } ?>	 
		          <?php 
		          $sql = "SELECT * from books where book_category ='child'  and  flag = 'yes'  order by created_at desc";
		          $result  = mysqli_query($conn,$sql);
		          $rows = mysqli_fetch_all($result,MYSQLI_ASSOC);
		          ?>         
