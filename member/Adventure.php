...
<body>
  ...
  <!-- adventure-books start -->
	        <div class="adventure-books mt-2 font-tit">
				   	<?php 
		          $sql = "SELECT book_category from books where book_category ='adventure' limit 1";
		    ...
		    <?php
		           ...
		           //while($row  = mysqli_fetch_assoc($result))
		            {     ?> 
					<h2 class="font-22 border-bottom font-lemon text-uppercase py-3 font-weight-bold" style="color:#DC143C;">
			         <?= $row['book_category'] ?> 			
					</h2>
				 <?php } ?>	 
		          <?php 
		          $sql = "SELECT * from books where book_category ='Adventure' and  flag = 'yes' order by created_at desc";
		          $result  = mysqli_query($conn,$sql);
		          $rows = mysqli_fetch_all($result,MYSQLI_ASSOC);
		          ?>         
