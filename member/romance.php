...
<body>
  ...
  <!-- adventure-books start -->
	        <div class="adventure-books mt-2 font-tit">
				   	<?php 
		          $sql = "SELECT book_category from books where book_category ='romance' limit 1";
		    ...
		    <?php
				 ...
				 <?php } ?>	 
		          <?php 
		          $sql = "SELECT * from books where book_category ='romance' and  flag = 'yes'   order by created_at desc";
		          $result  = mysqli_query($conn,$sql);
		          $rows = mysqli_fetch_all($result,MYSQLI_ASSOC);
		          ?>         
